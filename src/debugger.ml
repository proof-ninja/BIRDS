...
let string_of_summary_explanation (fact, edb_facts, all_clauses, detail) = 
      ...
      "\n    ~~~~~~~~~~~~~~~~~~~~~\n"^
      String.concat "" (List.map (fun clause -> "         || "^ (string_of_stt clause)) all_clauses) ^
      "         \\/  \n"^
      "    ~~~~~~~~~~~~~~~~~~~~~\n"^
      "    "^ (string_of_fact fact)^ "\n"
...
let gen_counterexample (log:bool) (property:string) (maxsize:int) (timeout:int) iprog =
    ...
    let string_adom = init_vocabulary@(Rule_preprocess.extract_rules_string_adom all_rules) in
    ...
    let prog = Prog (non_rules@(Rule_preprocess.string2int_rules string_to_int all_rules)) in
    if log then (
    ...
    let rec gen_ctex i maxsize = 
      ...
      if (!error_mess = "") then
        ...
        if log then (print_endline ("\n________"^property^" counterexample________ ") ;
            print_endline (string_of_prog (Prog facts));
...
let is_missing_fact fact = match fact with 
    ...
    | _ -> invalid_arg "function is_missing_fact called without a fact"
    ...
    if (List.length explanation = 0) then 
        ...
        if using_fact then                     
            ...
            print_endline ("Type a number from 1 to "^string_of_int (List.length all_clauses )  ^ "to choose one rule in: \n" ^
                String.concat "" (List.mapi (fun num clause -> colored_string "red" (string_of_int (num+1) ^ ": "^string_of_stt clause)) all_clauses) ^
                " (type others to exit)");
            ...
            let inspect_rule i = 
                ...
                print_endline ("Inspecting "^ string_of_stt clause) ;
                print_endline ("    "^
                    String.concat " " (List.map string_of_fact premises)^
                    "\n    ------------------ "^ colored_string "red" (string_of_stt clause)^
                ...
                let upper_program = (Prog ((get_schema_stts prog)@(Stratification.get_preceding_rules prog head))) in 
                let upper_facts,_ = Evaluation.eval false [] (Expr.add_stts (get_all_facts buggy_prog) upper_program) in
...
let check_constraints (log:bool) prog =
    ...
    if log then (print_endline "\n_______ derived facts _____" ;
        print_endline (string_of_prog (Prog resulted_facts));
        print_endline "________________");
...
let explain_getput (log:bool) prog =
    ...
    let explanation = List.filter (fun (fact, edb_facts, all_clauses, detail) -> 
                let init_facts = get_all_facts buggy_prog in
    ...
    if (List.length explanation = 0) then 
        ...
        if (List.length source_facts > 0) then (
            List.iter (fun x -> print_string "| "; print_endline (string_of_fact x)) source_facts
        ) 
        else
            print_endline "| empty";
        print_endline "+-------------------------------------------+";
        ...
        if (List.length delta_facts > 0) then (
            List.iter (fun x -> print_string "| "; print_endline (string_of_fact x)) delta_facts
        ) 
        else
            print_endline "| empty";
        print_string "|      ";
        ...
        List.iter (fun x -> print_string "|         ";
            print_endline (colored_string "" (string_of_fact x))) 
    ...
    if (List.length explanation = 0) then 
        ...
        if using_fact then                     
            ...
            let inspect_rule i = 
                ...
                print_endline "Please change the rule such that";
                let head = rule_head clause in 
                let upper_program = (Prog ((Stratification.get_preceding_rules buggy_prog head))) in 
                let upper_facts,_ = Evaluation.eval false [] (Expr.add_stts (get_all_facts buggy_prog) upper_program) in
                let upper_facts = List.filter (fun x -> (Lib.non is_missing_fact x) && (Lib.non is_builtin_fact x)  ) upper_facts in
                let derived_facts = extract_facts resulted_facts (get_rterm_relname head) in
                print_endline ("    "^
    ...
    if ((List.length wrong_tuples_explanation + List.length new_view_missing_facts) = 0) then 
        ...
        if using_fact then                     
            ...
            let inspect_rule i = 
                ...
                print_endline "Please change the rule such that";
                let head = rule_head clause in 
                let upper_program = (Prog ((get_schema_stts buggy_putget_prog)@(Stratification.get_preceding_rules buggy_putget_prog head))) in 
                let upper_facts,_ = Evaluation.eval false [] (Expr.add_stts (get_all_facts buggy_putget_prog) upper_program) in
                let upper_facts = List.filter (fun x -> (Lib.non is_missing_fact x) && (Lib.non is_builtin_fact x)  ) upper_facts in
                let derived_facts = extract_facts resulted_facts (get_rterm_relname head) in
                print_endline ("    "^
                String.concat " " (List.map string_of_fact upper_facts)^
...
let explain_putget (log:bool) prog = 
    ...
    if ((List.length wrong_tuples_explanation + List.length new_view_missing_facts) = 0) then 
        ...
        if (List.length new_source_facts > 0) then (
            List.iter (fun x -> print_string "| "; print_endline (String.sub (string_of_fact x) 12 (String.length (string_of_fact x) -12) )) new_source_facts
        )
        else
            print_endline "| empty";
        print_endline "+---------------------------------------------+";
        ...
        if (List.length new_view > 0) then (
            List.iter (fun x -> print_string "| "; print_endline (String.sub (string_of_fact x) 12 (String.length (string_of_fact x) -12))) new_view
        )
        else
            print_endline "| empty";
        print_string "|      ";
        ...
        ( print_endline "|      The following derived tuples are wrong:";
            List.iter (fun x -> print_string "|         ";
            print_endline (colored_string "" (String.sub (string_of_fact x) 12 (String.length (string_of_fact x) -12)))) 
            (List.map (fun (fact, edb_facts, all_clauses, detail) -> fact) wrong_tuples_explanation));
        if (List.length new_view_missing_facts > 0) then 
            (print_endline "|      The following tuples are missing:";
            List.iter (fun x -> print_string "|         ";
            print_endline (colored_string "" (String.sub (string_of_fact x) 12 (String.length (string_of_fact x) -12)))) new_view_missing_facts);
        print_endline "+--------------------------------------------------+";
    )
;;
