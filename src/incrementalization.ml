...
let incrementalization_ast (ast: expr): (rule list, error) result =
  ...
  match ast.view with
    | Some (view_name, params) ->
      ...
      (* let _ = print_endline @@ (sorted_predicate_definitions |>
        List.map (fun (impred, _) ->
          string_of_intermediate_predicate impred
        ) |> String.concat ", ")
      in *)
      ...
      let is_linear_occurrence = false (* TODO *) in

      (* let is_linear_view = has_anonymous_view_patermeters && is_linear_occurrence in *)
      let is_linear_view = has_anonymous_view_patermeters in
