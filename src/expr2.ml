...
type condition =
  | Condition of string * string * vterm
  | Exists of sql
  | Notexists of sql
and sql =
  | Select of (string list * string list * condition list)


type update =  (* update, insert, delete *)
  | Update of string * uset list * condition list 
  | Insert1 of string * const list
  | Insert2 of string * sql
  | Delete of string * condition list 
...
let add_stt stt expr = match stt with
  ...
  | Stt_Update update -> begin match expr.update with
                         | Some _ -> invalid_arg "Update should appear at most once"
                         | None -> {expr with update=Some update}
                         end
...
let string_of_uset (s1, s2) = s1 ^ "=" ^ string_of_vterm s2

let rec string_of_sql (s:sql) = match s with
  | Select (ss1, ss2, cs) -> 
     "SELECT " ^ String.concat ", " ss1 ^ " FROM " ^ String.concat ", " ss2 
     ^ " WHERE " ^ String.concat " AND " (map string_of_condition cs) ^ "\n"
and string_of_condition c = match c with     
  | Condition (op, e1,e2) -> e1 ^ " " ^ op ^ " " ^ (string_of_vterm e2)
  | Exists (sql) -> "EXISTS (" ^ (string_of_sql sql) ^ ")"
  | Notexists (sql) -> "NOT EXISTS (" ^ (string_of_sql sql) ^ ")"

let string_of_update (e:update) = match e with
  | Update (tname, ss, cs) -> "UPDATE " ^ tname ^ " SET "
                             ^  String.concat " , " (map string_of_uset ss) 
                             ^ " WHERE " ^ String.concat " AND " (map string_of_condition cs) ^ "\n"
  | Insert1 (tname, cs) -> "INSERT INTO " ^ tname ^ " VALUES (" ^ String.concat " , " (map string_of_const cs) ^")\n"
  | Insert2 (tname, sql) -> "INSERT INTO " ^ tname ^ " " ^ string_of_sql sql ^ "\n"
  | Delete (tname, cs) -> "DELETE FROM " ^ tname ^ " WHERE " ^ String.concat " AND " (map string_of_condition cs) ^ "\n"

(** smart stringify for AST *)
let to_string { rules; facts; query; sources; view; constraints; primary_keys; update;} : string = 
